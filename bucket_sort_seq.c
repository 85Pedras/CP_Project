...
long long values[NUM_EVENTS], min_values[NUM_EVENTS]; // PAPI counters' values

#define NUM_RUNS 5 // number of times the function is executed and measured
...
void bucket_sort (int v[],int size){         
    ...
    for(i=0;i<N_BUCKETS;i++) bs[i].bucket = malloc(BUCKET_SIZE*sizeof(int)); //aloca cada bucket
    ...
    /*for(i=0;i<N_BUCKETS;i++){ //Imprimir cada bucket
        printf("Bucket %d:\n",i);
        for(j=0;j<bs[i].n;j++){
            printf("%d\n",bs[i].bucket[j]);
        }
    ...
    printf("--------\n");*/                                             
    for(j=0,i=0;j<N_BUCKETS;j++){ //põe os elementos dos buckets de volta no vetor
        for(k=0;k<bs[j].n;k++){
            v[i]=bs[j].bucket[k];
            i++;
        }
